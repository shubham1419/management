...
public class ManagementController {
	...
	public ModelAndView showAllEmployees(@RequestParam(name = "status", required = false) String status) {
	...
	public ModelAndView showEmployeeEdit(@PathVariable int id) {

		ModelAndView mv = new ModelAndView("index");
		mv.addObject("userClickEmployeeEdit", true);
		mv.addObject("title", "Edit Employees");
	...
	public String showEmployeeUpdate(@ModelAttribute("employee") Employee memployee, HttpServletRequest request) {

		/*encoding the password*/
		memployee.setPassword(passwordEncoder.encode(memployee.getPassword()));	
		memployee.getEmployeeMeta().setEmployee(memployee);
		employeeDao.update(memployee);
		return "redirect:/manage/all-employees?status=empUpdate";
	}
	 ...
	 * public ModelAndView showManageProjectTasks(@RequestParam(name="status",
	 * required= false) String status) { ModelAndView mv = new
	 * ModelAndView("index"); mv.addObject("userClickAddProjectTasks", true);
	 * mv.addObject("title", "Add Project Tasks"); ProjectsMeta nprojectMeta =
	 * new ProjectsMeta(); nprojectMeta.setStatus(true);
	 * mv.addObject("projectMeta", nprojectMeta); if(status!= null) {
	 * if(status.equals("success")) {
	 * mv.addObject("message","Project Tasks Added Successfully");
	 * mv.addObject("msgclass","success"); } } return mv; }
	 * 
	 * 
	 * /*for post request*
	 * 
	 * @RequestMapping(value="add/project/tasks", method =RequestMethod.POST)
	 * public String submitProjectTasks(@ModelAttribute("projectMeta")
	 * ProjectsMeta mprojectMeta, HttpServletRequest request) {
	 * logger.info(mprojectMeta.toString()); projectsmetaDao.add(mprojectMeta);
